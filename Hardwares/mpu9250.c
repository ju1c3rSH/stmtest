...
{
	spi_w_bytes(&hspi1, address, &byte, 1);
}
 ...
 * @brief   init mpu9250
 * */
uint8_t MPU9250_Init(MPU9250 *mpu, SPI_HandleTypeDef *hspi, GPIO_TypeDef *cs_port, uint16_t cs_pin)
{
	MPU9250_StructInit(mpu);

	/* INIT PROPS BEGIN*/
	mpu->mpu_prop.hspi = hspi;
	mpu->mpu_prop.cs_gpio_pin = cs_pin;
	mpu->mpu_prop.cs_gpio_port = cs_port;
	...
	mpu_w_reg(ACCEL_CONFIG, (uint8_t)MPU9250_Accel_Range_4G);	 // accel full scale select
		mpu_w_reg(CONFIG, (uint8_t)MPU9250_Gyro_DLPFBandwidth_41);
...
{	//4g量程
	...
	mpu->mpu_data.Accel[0] = (float)mpu->mpu_data.Accel_row[0] * accel_scale;

	// calculate y axis
	mpu->mpu_data.Accel_row[1] = ((int16_t)dataBuf[2] << 8) | dataBuf[3];
	mpu->mpu_data.Accel[1] = (float)mpu->mpu_data.Accel_row[1] * accel_scale;

	// calculate z axis
	mpu->mpu_data.Accel_row[2] = ((int16_t)dataBuf[4] << 8) | dataBuf[5];
	mpu->mpu_data.Accel[2] = (float)mpu->mpu_data.Accel_row[2] * accel_scale;
}
/*
...
void MPU9250_ReadMag(MPU9250 *mpu)
{
	...
	mpu->mpu_data.Magn[0] = (float)mpu->mpu_data.Magn_row[0] * (((mag_adjust[0] - 128) / 256.0) + 1);
	mpu->mpu_data.Magn[0] = mpu->mpu_data.Magn_row[0] * 0.15;

	mpu->mpu_data.Magn[1] = (float)mpu->mpu_data.Magn_row[1] * (((mag_adjust[1] - 128) / 256.0) + 1);
	mpu->mpu_data.Magn[1] = mpu->mpu_data.Magn_row[1] * 0.15;

	mpu->mpu_data.Magn[2] = (float)mpu->mpu_data.Magn_row[2] * (((mag_adjust[2] - 128) / 256.0) + 1);
...
uint8_t MPU9250_ConfigureClock(
	MPU9250 *mpu)
{
	if (mpu == NULL)
	{
	...
	mpu_w_reg(PWR_MGMT_1, 0x01); // Auto selects the best available clock source – PLL if ready, else use the Internal oscillator
	mpu_w_reg(PWR_MGMT_2, 0x00); // Enable Accelerometer and Gyroscope
	return 1;
}

uint8_t MPU9250_ConfigureMagnetometer(
	MPU9250 *mpu)
{
	if (mpu == NULL)
	{
	...
	mpu_w_reg(USER_CTRL, 0x20);				  // Enable I2C Master mode
	mpu_w_reg(I2C_MST_CTRL, 0x0D);			  // I2C configuration multi-master 400kHz
	mpu_w_ak8963_reg(AK8963_CNTL1_REG, 0x00); // Power down magnetometer
	HAL_Delay(100);
	mpu_w_ak8963_reg(AK8963_CNTL1_REG, 0x0F); // Enter Fuse ROM access mode
	HAL_Delay(100);
	mpu_w_ak8963_reg(AK8963_CNTL1_REG, 0x00); // Power down magnetometer
	HAL_Delay(100);
	mpu_w_ak8963_reg(AK8963_CNTL1_REG, 0x16); // Set magnetometer to continuous measurement mode 2 (100Hz) and 16-bit output
	HAL_Delay(100);
