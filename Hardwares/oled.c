...
// #define OLED_LOCK()     do { if (oled_mutex) osMutexAcquire(oled_mutex, osWaitForever); } while(0)
// #define OLED_UNLOCK()   do { if (oled_mutex) osMutexRelease(oled_mutex); } while(0)
// osMutexId_t oled_mutex; //OLED操作互斥锁
...
static OLED_PropTypeDef m_oled_props = {
	.sda_gpio_port = NULL,
	.scl_gpio_port = NULL};

static void OLED_SDA_Set(uint8_t val)
{
	if (val)
		HAL_GPIO_WritePin(m_oled_props.sda_gpio_port, m_oled_props.sda_gpio_pin, GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(m_oled_props.sda_gpio_port, m_oled_props.sda_gpio_pin, GPIO_PIN_RESET);
}
static void OLED_SCL_Set(uint8_t val)
{

	if (val)
		HAL_GPIO_WritePin(m_oled_props.scl_gpio_port, m_oled_props.scl_gpio_pin, GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(m_oled_props.scl_gpio_port, m_oled_props.scl_gpio_pin, GPIO_PIN_RESET);
}
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	if (!m_oled_props.scl_gpio_port)
		return;
	//	GPIO_WriteBit(GPIOC, GPIO_Pin_2, (BitAction)BitValue);
	HAL_GPIO_WritePin(m_oled_props.scl_gpio_port, m_oled_props.scl_gpio_pin, BitValue);
	// HAL_GPIO_WritePin(m_oled_props.scl_gpio_port,
				 // m_oled_props.scl_gpio_pin, 
                 // (GPIO_PinState)BitValue); // 添加强制转换
				  // DWT_Delay_us(2);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	if (!m_oled_props.sda_gpio_port)
		return;
	//	GPIO_WriteBit(GPIOC, GPIO_Pin_0, (BitAction)BitValue);
	HAL_GPIO_WritePin(m_oled_props.sda_gpio_port, m_oled_props.sda_gpio_pin, BitValue);
	//	Delay_us(1);
	// DWT_Delay_us(2);
...
{
	m_oled_props = *OLED_Prop;

	const osMutexAttr_t mutex_attr = {
		.name = "OLED_Mutex",
		.attr_bits = osMutexRecursive // 允许同一线程多次加锁
	};
...
{
	// if (oled_mutex) osMutexAcquire(oled_mutex, osWaitForever);
	uint8_t j;
	/*遍历每一页*/
	for (j = 0; j < 8; j++)
	{
	...
	// if (oled_mutex) osMutexRelease(oled_mutex);
}
