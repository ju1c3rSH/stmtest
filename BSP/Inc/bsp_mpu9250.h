...
//#define MPU_ACCEL_OFFS_REG		0X06	//accel_offs寄存�?可读取版本号,寄存器手册未提到
//#define MPU_PROD_ID_REG			0X0C	//prod id寄存�?在寄存器手册未提�?
...
#define MPU_PWR_MGMT1_REG		0X6B	//电源管理寄存�?
#define MPU_PWR_MGMT2_REG		0X6C	//电源管理寄存�? 
#define MPU_FIFO_CNTH_REG		0X72	//FIFO计数寄存器高八位
#define MPU_FIFO_CNTL_REG		0X73	//FIFO计数寄存器低八位
#define MPU_FIFO_RW_REG			0X74	//FIFO读写寄存�?
#define MPU_DEVICE_ID_REG		0X75	//器件ID寄存�?
 
//如果AD0�?9�?接地,IIC地址�?X68(不包含最低位).
//如果接V3.3,则IIC地址�?X69(不包含最低位).
...
////因为模块AD0默认接GND,所以转为读写地址�?�?XD1�?XD0(如果接VCC,则为0XD3�?XD2)  
//#define MPU_READ    0XD1
//#define MPU_WRITE   0XD0

uint8_t MPU_Init(void); 								//初始化MPU6050
uint8_t MPU_Write_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf);//IIC连续�?
uint8_t MPU_Read_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf); //IIC连续�?
uint8_t MPU_Write_Byte(uint8_t reg,uint8_t data);				//IIC写一个字�?
uint8_t MPU_Read_Byte(uint8_t reg);						//IIC读一个字�?
